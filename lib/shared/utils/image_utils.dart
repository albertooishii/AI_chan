...
Future<String?> saveBase64ImageToFile(
  ...
  try {
    ...
    if (exists) {
      Log.i(
        '[Image] Imagen guardada correctamente en: $absFilePath',
        tag: 'IMAGE_UTILS',
      );
      return fileName;
    } else {
...
Future<Directory> getLocalImageDir() async {
  ...
  if (configured.trim().isEmpty) {
    throw StateError(
      'IMAGE_DIR no está configurado en .env para esta plataforma',
    );
  }
  ...
  if (cfg.startsWith('~')) {
    if (home.isEmpty) {
      throw StateError(
        'No se pudo determinar HOME para expandir ~ en IMAGE_DIR_DESKTOP',
      );
    }
