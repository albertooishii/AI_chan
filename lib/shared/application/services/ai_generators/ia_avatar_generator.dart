...
class IAAvatarGenerator {
  ...
  Map<String, dynamic> _createBasePrompt() {
    return {
      ...
      'salida':
          'Usa la herramienta de generación de imágenes y devuelve únicamente el nombre de archivo local (`file_name`) o la ruta relativa del fichero generado.',
      'notas':
  ...
  Future<AIResponse> _generateImageWithRetries(
    ...
    for (int attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        // Considerar únicamente el fileName devuelto por el manager
        if (response.imageFileName.isNotEmpty) {
          final source = 'imageFileName';
          Log.d(
            '[IAAvatarGenerator] Avatar: imagen obtenida en intento ${attempt + 1} (via $source)',
          );
          return response;
        }
  ...
  Future<AiImage> _saveAndCreateAvatar(
    ...
    // El guardado del binario (base64 -> fichero) lo realiza el `AIProviderManager`
    // que persiste cualquier payload base64 devuelto por el provider y expone
    // el `imageFileName` en el `AIResponse`. Aquí sólo construimos el `AiImage`
    // usando el `imageFileName` que el manager dejó en `imageResponse`.
