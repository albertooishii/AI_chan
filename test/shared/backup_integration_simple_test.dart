...
void main() {
  ...
  group('Backup Integration - End to End Simulation', () {
    ...
    group('🔄 Complete Backup Cycle', () {
      test('should complete full backup workflow: prepare -> upload -> verify', () async {
        ...
        final mockChatHistory = {
          'messages': [
            {'id': 1, 'text': 'Hello AI-chan', 'timestamp': '2025-01-30T10:00:00Z'},
            {'id': 2, 'text': 'Hello human!', 'timestamp': '2025-01-30T10:00:01Z'},
          ],
        };

        final mockVoiceSettings = {'voice_model': 'cyberpunk_ai', 'speech_rate': 1.0, 'volume': 0.8};
        ...
        final mockHttpClient = MockClient((request) async {
          ...
          if (url.contains('/upload') && request.method == 'POST') {
            uploadCalled = true;
            return http.Response('{}', 200, headers: {'location': 'https://upload.googleapis.com/session/test123'});
          }

          // Mock resumable upload completion
          if (url.contains('upload.googleapis.com') && request.method == 'PUT') {
        ...
        final uploadRequest = http.Request('POST', Uri.parse('https://www.googleapis.com/upload/drive/v3/files'));
        ...
        // Complete upload
        final completeRequest = http.Request('PUT', Uri.parse('https://upload.googleapis.com/session/test123'));
        ...
        expect(uploadResult['size'], equals(backupFile.lengthSync().toString()));
      });
      ...
      test('should handle backup with media files simulation', () async {
        ...
        final totalSize = chatBackup.lengthSync() + voiceBackup.lengthSync() + mediaFile.lengthSync();
    ...
    group('🚨 Error Recovery Scenarios', () {
      test('should handle network interruption during upload', () async {
        ...
        final retryClient = MockClient((request) async {
          ...
          if (request.url.toString().contains('/upload') && request.method == 'POST') {
            return http.Response('{}', 200, headers: {'location': 'https://upload.googleapis.com/session/retry123'});
          }

          if (request.url.toString().contains('upload.googleapis.com')) {
            return http.Response(jsonEncode({'id': 'recovered_backup_123'}), 200);
          }
        ...
        try {
          final request = http.Request('POST', Uri.parse('https://www.googleapis.com/upload/drive/v3/files'));
        ...
        final retryRequest = http.Request('POST', Uri.parse('https://www.googleapis.com/upload/drive/v3/files'));
    ...
    group('🧹 Cleanup and Management', () {
      ...
      test('should calculate storage usage', () {
        final backupSizes = [1024000, 2048000, 512000]; // Different backup sizes in bytes
