...
Future<File> _fakeLocalBackupCreator({required String jsonStr, String? destinationDirPath}) async {
  // Create a tiny temp file to simulate the zip
  final dir = Directory(destinationDirPath ?? Directory.systemTemp.path);
  if (!dir.existsSync()) dir.createSync(recursive: true);
  final f = File('${dir.path}/fake_backup_${DateTime.now().millisecondsSinceEpoch}.zip');
...
void main() async {
  ...
  test('loadAll triggers auto-backup when there are messages but no recent backup (daily policy)', () async {
    ...
    BackupAutoUploader.testGoogleBackupServiceFactory = () => _FakeGoogleSvc(onUpload: () => called = true);

    // Populate messages to simulate non-empty chat
    provider.messages = [
      Message(text: 'hi', sender: MessageSender.user, dateTime: DateTime.now(), status: MessageStatus.read),
    ];

    // Persist Google linked in prefs so loadAll fallback branch keeps googleLinked=true
    await PrefsUtils.setGoogleAccountInfo(email: 'a@b', avatar: null, name: 'a', linked: true);
  ...
  test('loadAll triggers auto-backup when messages empty and no previous backup', () async {
    final provider = ChatProvider();
    provider.googleLinked = true;
    provider.onboardingData = AiChanProfile(
      userName: 'T',
    ...
    BackupAutoUploader.testLocalBackupCreator = _fakeLocalBackupCreator;
    BackupAutoUploader.testGoogleBackupServiceFactory = () => _FakeGoogleSvc(onUpload: () {});
  ...
  test('triggers auto-backup on various chat actions when googleLinked', () async {
    ...
    final calls = <String>[];
    BackupAutoUploader.testLocalBackupCreator = _fakeLocalBackupCreator;
    BackupAutoUploader.testGoogleBackupServiceFactory = () => _FakeGoogleSvc(onUpload: () => calls.add('upload'));
    ...
    await provider.updateOrAddCallStatusMessage(text: 'call ended', callStatus: CallStatus.completed, incoming: false);
    await Future.delayed(const Duration(milliseconds: 400));
    ...
    provider.rejectIncomingCallPlaceholder(index: provider.messages.length - 1, text: 'rejected');
    await Future.delayed(const Duration(milliseconds: 200));
  ...
  test('generates summary block after SUMMARY_BLOCK_SIZE+1 messages and triggers backup', () async {
    final provider = ChatProvider();
    provider.googleLinked = true;
    provider.onboardingData = AiChanProfile(
      userName: 'T',
