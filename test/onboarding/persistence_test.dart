...
void main() {
  ...
  test('onboarding+chat persistence -> save/import roundtrip and export JSON', () async {
    ...
    final profile = AiChanProfile(
      ...
      timeline: timeline,
      avatars: [avatar],
      events: [EventEntry(type: 'promesa', description: 'Hacer backup', date: DateTime.now())],
    );
    ...
    final imported = ImportedChat(profile: profile, messages: [m1, m2], events: events);
    ...
    final parsed = await AiChanProfile.tryFromJson(bioMap as Map<String, dynamic>);
    ...
    final export = ChatExport(profile: profile, messages: [m1, m2], events: events);
  ...
  test('persistence survives reload: loadAll restores onboarding/messages/events', () async {
    ...
    final m1 = Message(text: 'Hola recarga', sender: MessageSender.user, dateTime: DateTime.now());
    final m2 = Message(text: 'Respuesta', sender: MessageSender.assistant, dateTime: DateTime.now());
    final events = [EventEntry(type: 'cita', description: 'Dentista', date: DateTime.now())];
  ...
  test('ChatJsonUtils.importAllFromJson accepts valid JSON and rejects corrupted', () async {
    ...
    final m1 = Message(text: 'Hola', sender: MessageSender.user, dateTime: DateTime.now());
    final m2 = Message(text: 'Adios', sender: MessageSender.assistant, dateTime: DateTime.now());
    final events = [EventEntry(type: 'cita', description: 'Prueba', date: DateTime.now())];
    ...
    String? errMsg;
    final importedGood = await chat_json_utils.ChatJsonUtils.importAllFromJson(
      jsonEncode(goodMap),
      onError: (e) => errMsg = e,
    );
    ...
    errMsg = null;
    final badStruct = jsonEncode({'userName': 'x', 'aiName': 'y', 'biography': {}, 'appearance': {}});
    final importedBad = await chat_json_utils.ChatJsonUtils.importAllFromJson(badStruct, onError: (e) => errMsg = e);
    expect(importedBad, isNull);
    expect(errMsg, isNotNull);

    // Caso: JSON malformado
    errMsg = null;
    final importedMalformed = await chat_json_utils.ChatJsonUtils.importAllFromJson(
      'not a json',
      onError: (e) => errMsg = e,
    );
