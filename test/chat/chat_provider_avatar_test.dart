...
void main() async {
  ...
  test(
    'createAvatarFromAppearance appends avatar and system message on success',
    () async {
      ...
      try {
        ...
        expect(
          provider.messages.any((m) => m.sender == MessageSender.system),
          true,
        );
      } finally {
  ...
  test(
    'createAvatarFromAppearance retries when first attempt fails and user chooses retry',
    () async {
  ...
  test(
    'createAvatarFromAppearance with replace=true replaces avatar and does not add system message',
    () async {
      final provider = TestChatProvider(dialogResult: 'cancel');
      provider.onboardingData = AiChanProfile(
        events: [],
