    ...
    test('domain interfaces should be abstract or have abstract methods', () {
      ...
      for (final file in domainDir.listSync(recursive: true)) {
        if (file is File &&
          ...
          final concreteClassPattern = RegExp(
            r'class\s+(\w+)(?!\s+extends|\s+implements)',
          );
          ...
          for (final match in matches) {
            ...
            if ((className.endsWith('Service') ||
              ...
              violations.add(
                '${file.path}: $className should be abstract interface (domain layer)',
              );
            }
            ...
            if (abstractMethods > 0 &&
              ...
              violations.add(
                '${file.path}: $className has only abstract methods but is not marked as abstract',
              );
            }
          }
        }
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Domain layer concrete class violations:\n${violations.join('\n')}',
      );
    });
    ...
    test('adapters should implement domain interfaces', () {
        ...
        if (file is File && file.path.endsWith('.dart')) {
          ...
          if (file.path.contains('/domain/interfaces/') ||
            ...
            final matches = RegExp(
              r'abstract (?:interface )?class\s+(\w+)',
            ).allMatches(content);
            for (final match in matches) {
      ...
      for (final adapter in adapterFiles) {
        ...
        if (!implementsDomainInterface) {
          violations.add(
            '${adapter.path}: Adapter does not implement any known domain interface',
          );
        }
      }
      ...
      final ratio =
          adapterFiles.length /
          (domainInterfaces.isNotEmpty ? domainInterfaces.length : 1);
      if (ratio > 3.0) {
        violations.add(
      ...
      expect(
        violations,
        isEmpty,
        reason: 'Adapter implementation violations:\n${violations.join('\n')}',
      );
    });
    ...
    test('presentation layer should depend on domain, not infrastructure', () {
      ...
      for (final file in libDir.listSync(recursive: true)) {
        if (file is File &&
          ...
          final infrastructurePattern = RegExp(
            'import\\s+["\'].*/(infrastructure|adapters)/',
          );

          if (infrastructurePattern.hasMatch(content)) {
            violations.add(
              '${file.path}: Presentation layer imports infrastructure directly',
            );
          }
        }
      }
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Presentation importing infrastructure violations:\n${violations.join('\n')}',
      );
    });
  });
}
