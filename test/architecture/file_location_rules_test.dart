  ...
  group('File location rules', () {
    test('no layer folders nested inside domain/infrastructure/presentation', () {
      ...
      expect(
        violations,
        isEmpty,
        reason: 'Reglas de ubicación violadas:\n${violations.join('\n')}',
      );
    });

    test(
      'no files with forbidden suffixes or generated files in source folders',
      () {
        final libDir = Directory('lib');
        if (!libDir.existsSync()) return;
        ...
        for (final f in files) {
          ...
          if (path.contains('/domain/') && path.endsWith('.g.dart')) {
            violations.add(
              '$path -> Archivo generado dentro de domain (revisa ubicación)',
            );
          }
          ...
          if (path.contains('/build/') || path.contains('/.dart_tool/')) {
            violations.add(
              '$path -> Artefacto de build presente dentro de lib',
            );
          }
        }
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Archivos en ubicaciones prohibidas:\n${violations.join('\n')}',
        );
      },
    );
  });
